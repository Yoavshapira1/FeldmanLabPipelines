...
#creates surrogate connectivity data for all combinations of a participant from one dyad with the 2nd participant with each of the other dyads
...
# -----------------------------------------------------------------------------
PATH_DIR_SRC = 'source folder with epochs after AR - same folder used to calculate real connectivity/' #ICA clean
PATH_DIR_DEST = "output folder for surrogate data/"
PATH_DURATION_CSV = 'xlsx file used in the regular connectivity script'
#was just ica ver 4, not replication before

# duration dictionary
SBJ_COL_NAME = 'subject_ID' 
DUR_COL_NAME = 'min' #could be minumum or face and then -110

#DUR_COL_NAME = 'minimum'
OFFSET_START = 2
OFFSET_END = 2 
...
def duplicate_condition_check(name1, name2) -> bool:
    if 'mom' in name1 and 'mom' in name2:
        return False
...
mother_ls = [file for file in file_ls if 'mom' in file]
...
for (subj_1, subj_2) in file_ls:
    ...
    if clean_count < 120 : #was 120 in rest, 40 in previous analysis. now 2.5 sd from mean
        print(f"Skipping pair {subj_1[:11]} and {subj_2[:11]}: insufficient clean epochs ({clean_count})")
        continue
    ...
    df = pd.DataFrame(
        dict(
            ...
            **{key: np.reshape(connB[:, :, idx], 62**2) for idx, key in enumerate(freq_bands.keys())}
        )
    )
    df = df.loc[(df['alpha'] > 0) | (df['beta'] > 0)]
